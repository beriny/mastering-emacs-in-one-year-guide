...
比如说自由度0, 即"无论用户出于何种目的，必须可以按照用r意愿，可以随时随处自由地运行该软件".很简单是吗?
很多人抱怨[[https://elpa.gnu.org]]不是很稳定,以至于...

#+begin_src bash
curl -L https://elpa.gnu.org/packages/archive-contents | sed 's/^(1//g' | sed ':a;N;$!ba;s/\n/ /g' | sed 's/\])/])\n/g' | sed 's/^ *(\([a-z0-9A-Z-]*\).*\[(\([0-9 ]*\).*\(single\|tar\).*/\1-\2.\3/g' | sed 's/ /./g' | sed 's/single/el/g' | tr --delete ')' | xargs -i{} curl -L  -O https://elpa.gnu.org/packages/{}
#+end_src
...
一个新手爱问的问题暴露了他们没有读官方教程.就是"我按了某某键,Emacs没有象我预期的工作".读过官方教程的人就知道每个快捷键对应一个命令,快捷键提供的信息接近于零.
